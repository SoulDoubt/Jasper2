...
bool GameObject::ShowGui()
{
	...
	{
		if (cmpidx == 1) {
			SkeletonComponent* skel = GetComponentByType<SkeletonComponent>();
			if (skel) {
				auto anim = AttachNewComponent<AnimationComponent>("Animator");
				anim->SetSkeleton(skel->GetSkeleton());
			}
	...
	for (auto& cmp : m_components) {
		if (ImGui::TreeNode((void*)(uintptr_t)i, cmp->GetName().data())) {
			...
			auto ctype = cmp->GetComponentType();
			if (ctype == ComponentType::AnimationComponent) {
				string title = "Animation Editor - " + GetName();
				ImGui::Begin(title.c_str());
				cmp->ShowGui();
				ImGui::End();
				
			}
			else if (ctype == ComponentType::SkeletonComponent) {
				string title = "Pose Editor - " + GetName();
				ImGui::Begin(title.c_str());
				cmp->ShowGui();
				ImGui::End();
			}
...
unique_ptr<GameObject> GameObject::DetachChild(const GameObject* child)
{
	...
	/*auto ref = m_children.erase(
		std::remove_if(
			m_children.begin(),
			m_children.end(),
			[&](const unique_ptr<GameObject>& ch) {
				return ch.get() == child;
			}
		),
		m_children.end()
	...
	auto ref = std::find_if(
		m_children.begin(),
		m_children.end(),
		[&](unique_ptr<GameObject>& ch) {
			bool match = ch.get() == child;
			return match;
	}
	);

	if (ref != m_children.end()) {
		auto detached = move(*ref);
		m_children.erase(ref);		
		return move(detached);
	}
