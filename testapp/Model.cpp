...
ImporterSceneNode* ModelLoader::GetRootBoneNode(Skeleton* skeleton) {
	const auto& bd = skeleton->Bones[0];
...
void ModelLoader::LoadModel(const std::string& filename, const std::string& name)
{
	...
	// ensure that we have a local copy of the assimp scene before mesh provessing
	...
	if (model_data->GetSkeleton()->Bones.size() > 0) {
		...
		auto rootBoneNode = GetRootBoneNode(skeleton);
	...
	if (cachedMesh) {
		...
		if (aiMesh->HasBones()) {
			...
			for (uint i = 0; i < aiMesh->mNumBones; ++i) {
				...
				auto isn = FindImporterSceneNode(bname);
				bd.BoneTransform = isn->NodeTransform;
				auto pnode = isn->Parent;
				while (pnode != nullptr) {
					bd.BoneTransform = pnode->NodeTransform * bd.BoneTransform;
					pnode = pnode->Parent;
				}
